...
//
// Note: the resolver addresses passed to SetUnhealthy are stored in the health
// tracker's internal state but cannot be retrieved without a health package API
// change, since dnsForwarderFailing has a TimeToVisible delay that defers the
// eventbus publication. The boolean IsUnhealthy check is sufficient to verify
// the critical path is exercised.
